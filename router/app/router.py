...
async def chat_completions(body: ChatCompletionRequest, request: Request):
    ...
    if body.stream:
        return StreamingResponse(
            _stream_with_tool_loop(client, settings, ollama_payload, tool_registry, resolved_model),
            media_type="text/event-stream",
            headers={"X-Accel-Buffering": "no"},
        )
    return await _complete_with_tool_loop(client, settings, ollama_payload, tool_registry, resolved_model)
...
def _make_sse_chunk(chunk_id: str, model: str, content: str, finish_reason) -> dict:
    return {
        "id": chunk_id,
